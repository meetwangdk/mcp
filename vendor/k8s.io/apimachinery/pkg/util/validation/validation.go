...
func IsValidLabelValue(value string) []string {
	var errs []string
	if utilfeature.DefaultFeatureGate.Enabled(features.DisableLabelValidation) {
		return errs
	}
	maxLabelValueLength := LabelValueMaxLength
	customLen, err := strconv.Atoi(os.Getenv("MAX_K8S_LABEL_LENGTH"))
	if err == nil && customLen > LabelValueMaxLength {
		maxLabelValueLength = customLen
	}
	if len(value) > maxLabelValueLength {
		errs = append(errs, MaxLenError(maxLabelValueLength))
	}
