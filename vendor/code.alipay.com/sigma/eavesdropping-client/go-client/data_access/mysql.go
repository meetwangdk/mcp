...
func (s *StorageSqlImpl) QueryEavesdroppingLatency(data interface{}, opts ...model.OptionFunc) error {
	return nil
}
func (s *StorageSqlImpl) QueryPodYamlTimelineWithPodUID(uid string, opts ...model.OptionFunc) (timeline []string, err error) {
	return
}
func (s *StorageSqlImpl) QueryPodYamlsV2WithPodUID(data interface{}, index, uid string, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryNodeYamlTimelineWithNodeUid(uid string, opts ...model.OptionFunc) (timeline []string, err error) {
	return
}
func (s *StorageSqlImpl) QueryNodeYamlTimelineWithNodeName(nodeName string, opts ...model.OptionFunc) (timeline []string, err error) {
	return
}
func (s *StorageSqlImpl) QueryNodeYamlsV2WithNodeName(data interface{}, index, name string, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryLivePodLatency(data interface{}, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QuerySpanWithPodUid(data interface{}, uid string, opts ...model.OptionFunc) error {
	options := &model.Options{
		Limit:     80,
		Ascending: false,
		OrderBy:   "span_elapsed",
	}
	if uid == "" {
	...
	tx := s.DB.Debug().Order(fmt.Sprintf("%s%s", options.OrderBy, order)).Limit(options.Limit).Where("or_uid = ?", uid).Find(data)
	if tx.Error != nil {
...
func (s *StorageSqlImpl) QueryLifePhaseWithPodUid(data interface{}, podUID string, opts ...model.OptionFunc) error {
	options := &model.Options{
		Limit:     200,
		Ascending: false,
		OrderBy:   "start_time",
	...
	tx := s.DB.Debug().Order(fmt.Sprintf("%s%s", options.OrderBy, order)).Limit(options.Limit).Where("pod_uid = ?", podUID).Find(data)
	if tx.Error != nil {
...
func (s *StorageSqlImpl) QueryPodYamlsWithPodUID(data interface{}, podUID string, opts ...model.OptionFunc) error {
	options := &model.Options{
		Limit:     5,
		Ascending: false,
		OrderBy:   "stage_timestamp",
	}
	for _, do := range opts {
		do(options)
	}
...
func (s *StorageSqlImpl) QueryPodYamlsWithPodName(data interface{}, podName string, opts ...model.OptionFunc) error {
	options := &model.Options{
		Limit:     5,
		Ascending: false,
		OrderBy:   "stage_timestamp",
	...
	tx := s.DB.Debug().Order(fmt.Sprintf("%s%s", options.OrderBy, order)).Limit(options.Limit).Where("pod_name = ?", podName).Find(data)
	if tx.Error != nil {
		klog.Errorf("db.Order Error: %s", tx.Error)
		return fmt.Errorf("error%v", tx.Error)
	}
...
func (s *StorageSqlImpl) QueryPodYamlsWithHostName(data interface{}, hostName string, opts ...model.OptionFunc) error {
	options := &model.Options{
		Limit:     5,
		Ascending: false,
		OrderBy:   "stage_timestamp",
	...
	tx := s.DB.Debug().Order(fmt.Sprintf("%s%s", options.OrderBy, order)).Limit(options.Limit).Where("hostname =?", hostName).Find(data)
	if tx.Error != nil {
		klog.Errorf("db.Order Error: %s", tx.Error)
		return fmt.Errorf("error%v", tx.Error)
	}
...
func (s *StorageSqlImpl) QueryPodYamlsWithPodIp(data interface{}, podIp string, opts ...model.OptionFunc) error {
	options := &model.Options{
		Limit:     5,
		Ascending: false,
		OrderBy:   "stage_timestamp",
	...
	tx := s.DB.Debug().Order(fmt.Sprintf("%s%s", options.OrderBy, order)).Limit(options.Limit).Where("pod_ip = ?", podIp).Find(data)
	if tx.Error != nil {
		klog.Errorf("db.Order Error: %s", tx.Error)
		return fmt.Errorf("error%v", tx.Error)
	}
...
func (s *StorageSqlImpl) QueryCreateSloWithResult(data interface{}, requestParams *model.SloOptions) error {
	...
	if requestParams.DeliveryStatus != "" {
		...
		if env == DELIVERY_ENV_TEST {
			tx = tx.Where("delivery_status_new =?", requestParams.DeliveryStatus)
		}
	...
	if requestParams.SloTime != "" {
		...
		if err == nil {
			...
			if env == DELIVERY_ENV_TEST {
				tx = tx.Where("delivery_slo_new=?", int(sloduration))
			}
			if env == DELIVERY_ENV_DRYRUN {
...
func (s *StorageSqlImpl) QueryPodLifePhaseByID(data interface{}, uid string, opts ...model.OptionFunc) error {

	return nil

}

func (s *StorageSqlImpl) QuerySloTraceDataWithPodRequestParams(data interface{}, requestParamsKey, requestParamsValue string) error {
	return nil
}
...
func (s *StorageSqlImpl) QueryObjectsFromTerms(result interface{}, index, doctype, filters, lte, gte string, size int) error {
	return nil
}
func (s *StorageSqlImpl) QueryObjectsFromNativeQuery(result interface{}, query, gte, lte, timeAttr string, size int) error {
	return nil
}
func (s *StorageSqlImpl) QueryObjectsFromNativeMultiQuery(result interface{}, item interface{}, query []string, gte, lte, timeAttr string, size int) error {
	return nil
}
func (s *StorageSqlImpl) QueryEavesdroppingMeta(data interface{}, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryLifePhaseWithDatasourceId(data interface{}, uid string, opts ...model.OptionFunc) error {
	return nil
}
func (s *StorageSqlImpl) QueryDeliveryWithDuration(data interface{}, params *model.SloOptions, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryLifePhaseWithNodeUid(data interface{}, uid string, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryLifePhaseWithNodeName(data interface{}, name string, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryNodeLifePhaseByID(data interface{}, uid string, opts ...model.OptionFunc) error {
	return nil
}
func (s *StorageSqlImpl) QueryWorkloadEventWithUid(data interface{}, Uid string, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryWorkloadEventWithName(data interface{}, Uid string, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) QueryAuditlogWithAuditId(data interface{}, Uid string, opts ...model.OptionFunc) error {
	return nil
}

func (s *StorageSqlImpl) InsertGatherFeedback(data interface{}) error {
	return nil
}

func (s *StorageSqlImpl) QueryFeedback(feedbackType []string, opts ...model.OptionFunc) (*elastic.SearchHits, error) {
	return nil, nil
}

func (s *StorageSqlImpl) UpdateGatherFeedback(id string, data interface{}) error {
	return nil
}

func (s *StorageSqlImpl) GetGatherFeedback(id string, data interface{}) error {
	return nil
}
